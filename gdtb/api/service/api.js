...
exports.GetApi = function (_req, _res, _callback, _senderror) {
  return {
    ...
    getParam: function (param, _code) {
      var code = _code || 4041;
      if (
        typeof _req.query[param] === "undefined" &&
        typeof _req.body[param] === "undefined"
      )
        throw code;
    ...
    createCase: function () {
      ...
      //通过token计算sign是否合法。
      var sign = common.md5(
        token.appkey + token.registno + token.timestamp + uuid + secretkey
      );
      console.log(
        token.appkey + token.registno + token.timestamp + uuid + secretkey
      );
      ...
      // 屏蔽写数据库，直接输出到 console
      console.log("uuid:", uuid);
      console.log("custid:", custid);
      console.log("registno:", token.registno);
      console.log("imagecount:", imagecount);
      console.log("token:", token);
      console.log("insurecode:", token.insurecode);
      console.log("insurename:", token.insurename);
      console.log("timestamp:", token.timestamp);

      return Me.res.send({ code: 200, msg: "success", data: "123" });
